...
for p in [5]:
    ...
    #Making thinning image
    # 細線化(スケルトン化) THINNING_ZHANGSUEN
    skeleton1 = cv2.ximgproc.thinning(binary, thinningType=cv2.ximgproc.THINNING_ZHANGSUEN)

    # 細線化(スケルトン化) THINNING_GUOHALL
    skeleton2 = cv2.ximgproc.thinning(binary, thinningType=cv2.ximgproc.THINNING_GUOHALL)
