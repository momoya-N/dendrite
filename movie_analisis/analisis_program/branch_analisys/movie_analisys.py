...
def branch_search(i, j):
    if i < len(position):
        ...
        if i_tmp == len(position) - 1:  # 中心の例外処理
            ...
            branch.append(
                [[position[i][j][1], position[i][j][0]], [position[i_tmp][j_tmp][1], position[i_tmp][j_tmp][0]]]
            )
            return
...
for path in file_path_list:
    ...
    branch_id = [
        1
    ]  # Pythonには参照渡しは存在しない(objectのaddressを値渡しする)ため、関数内ではコピーして渡されたaddressにアクセスしてobjectを書き換えることで参照渡しのような挙動を実現するらしい。
    ...
    for i in range(len(branch_vector_edge)):
        for j in range(i + 1, len(branch_vector_edge)):
            ...
            if not set(tmp1).isdisjoint(set(tmp2)):
                ...
                length = math.sqrt(
                    pow(rev1[1], 2) + pow(rev2[1], 2) - 2 * rev1[1] * rev2[1] * math.cos(rev1[0] - rev2[0])
                )
    ...
    Short_branch_vector = copy.deepcopy(
        branch_vector
    )  # copy methodを使わないと、Pythonの場合リストは"代入"ではなく、"同じリストオブジェクトの参照"の意味になる。
    ...
    for i in range(len(fig_list)):
        if i % 2 == 0:  # pdf_image
            path = Data_dir_path + fig_list[i]
            os.makedirs(path, exist_ok=True)
            plt.savefig(path + str(name_tag) + ".pdf")
        else:
            path = Data_dir_path + fig_list[i]
            os.makedirs(path, exist_ok=True)
            plt.savefig(path + str(name_tag) + ".png")

    for i in range(len(data_dir_list)):
        path = Data_dir_path + data_dir_list[i]
        if i % 2 == 0:  # length
            os.makedirs(path, exist_ok=True)
            path = path + fname
