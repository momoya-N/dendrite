  ...
  \item 濃度とCMCについて、
        \begin{figure}[H]
          \begin{minipage}[H]{0.49\columnwidth}
            \centering
            \includegraphics[width=0.5\textwidth]{figure/surface_tesion_tempreture.png}
            \caption{TWEEN系列の界面張力の濃度依存性(30\si{\degreeCelsius})\cite{kothekar2007comparative}}
            \label{fig:figure1}
          \end{minipage}
          \begin{minipage}[H]{0.49\columnwidth}
            \centering
            \includegraphics[width=0.5\textwidth]{figure/TWEEN_CMC.png}
...
\begin{equation}
  \begin{split}
    f_X(x)&=Cx^{-\alpha}\\
    \bar{F}_X(x)&=\int_{x}^{+\infty}Cx'^{-\alpha}\d x'=C\left[-\frac{1}{\alpha-1}x^{1-\alpha}\right]_{x}^{+\infty}\\
    &=\frac{C}{\alpha-1}x^{1-\alpha} \qquad(\alpha>1)
  \end{split}
\end{equation}
...
\begin{equation}
  \begin{split}
    f_X(x)&=C e^{-\alpha x}\\
    \bar{F}_X(x)&=\int_{x}^{+\infty}C e^{-\alpha x'}\d x'=C\left[-\frac{1}{\alpha}e^{-\alpha x'}\right]_{x}^{+\infty}\\
    &=\frac{C}{\alpha}e^{-\alpha x} \qquad(\alpha>0)
  \end{split}
\end{equation}
