...
\begin{figure}[htbp]
  ...
  \end{minipage}
  \caption{様々な非平衡系でのパターン形成。\subref{fig:BZ}Belousov-Zhabotinsky反応によるスパイラルパターン\cite{BZ_reaction}。直径おおよそ$\SI{50}{mm}$。\subref{fig:reaction_diffusion_angelfish}サザナミヤッコ（キンチャクダイの一種）の体表のTuringパターン。写真は横幅が$\SI{40}{mm}$から$\SI{50}{mm}$\cite{kondo1995reaction}。\subref{fig:Benard_cell}熱対流によるB{\'e}nard セル。円の直径は$\SI{120}{mm}${\cite{eckert1998square}}。}
