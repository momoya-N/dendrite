...
\end{equation}
で与えられる。（後の計算のため$\varepsilon$を比誘電率$\varepsilon_r$と真空の誘電率$\varepsilon_0$の積で表した。詳しい導出は付録\Red{??}参照。）
...
\end{equation}
で与えられる。$\gamma$は界面エネルギー密度，$\omega_0$は一分子当たりの体積である。大雑把には界面エネルギーと分子運動によるエネルギーの比を表し，長さの次元を持つ。\Red{$k_BT$の定義はいる？}
