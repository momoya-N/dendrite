...
\begin{document}
\section{議論・考察}
界面活性剤の濃度を増加させて金属樹を生成すると，濃度の増加と共に，傾向としてフラクタル次元が減少する結果が得られた（図\ref{fig:fractal_dim}）。この結果は金属樹の枝分かれが少なくなり（図\ref{fig:branch}\subref{fig:branch_number}），パターンが疎になることで一次元形状（線）の割合が増えたためだと考えられる。
