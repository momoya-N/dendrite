...
\label{sec:RW}
溶液中で粒子が拡散する際，その運動は\textbf{ランダムウォーク(Random Walk: RW)}でモデル化される。RWは粒子がある確率に従ってランダムに移動する運動である。そこで，$d$次元空間内でRWする場合を考える。\Red{ただし，本節の計算は文献\cite{フラクタル科学}\cite{フラクタルの物理Ⅱ}を参考に計算を拡張した。}
\begin{figure}[htbp]
  \centering
