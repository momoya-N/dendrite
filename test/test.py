...
# 解析開始・終了点の設定
def set_start_end_point(dci_thresh: float):  # dci_thresh: 単位 mA
    df = pd.read_excel(file_path_dci)
    print(df)
    time = df["#Time(sec.msec)"]
    dci = df["#DCI Value(A)"]
    dci_mA = dci * 1000
    delta_t = df["#delta_t(sec.msec)"]
    total_coulomb = pd.Series(df["#Q(C)"]).dropna()
    dci_mA_diff = np.gradient(dci_mA, time)
    # Print the time derivative of dci_mA
    print(total_coulomb)
    plt.plot(time, dci_mA_diff)
    sys.exit(1)
    # 解析開始フレームの設定
    start_frame = 0
    # 解析終了フレームの設定
...
for surafactant in surfactant_list:
    ...
    for file in file_list:
        ...
        start_frame, end_frame = set_start_end_point(1)
